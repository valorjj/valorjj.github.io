...
`OAuth 2.0`, `OpenID Connect Protocol`, `Keycloak`, `Okta` 등 에서는 비대칭키가 표준으로 사용된다. 중요한 데이터를 주고 받을 때는 비대칭키 방식을 사용하면 된다는 것만 기억하자. 대칭키에 비해서 암호화, 복호화 작업에 시간이 걸리는 성능 이슈가 있었지만 최근엔 많이 빨라졌다고 한다.
...
![bearer token filter](jwt/bearerTokenAuthenticationFilter.png)

스프링 시큐리티에서 모든 처리는 겹겹이 쌓인 filter 를 거친다. 인증 및 인가 과정에서 거부당한 요청은 filter 선에서 정리되어 컨트롤러까지 전달되지 않는다. 

`BearerTokenAuthenticationToken` 를 살펴보면, 일단 요청을 `Authentication` 객체로 감싼 다음에 `SecurityContext` 에 저장한다. 단순히 저장하는 과정이고 인증 과정이 일어나지는 않는다.
...
여기서 좀 생소한 단어는 `NimbusJWTDecoder` 이다. [여기](https://github.com/Connect2id/Nimbus-JWT) 에 간단하게 내용이 적혀있다.
...
![bearer token flow with auth server](jwt/bearerToken%20flow.png)
