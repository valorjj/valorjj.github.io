...
```
조금 과한 듯 하지만, 관리해야할 서비스가 점점 증가하는 상황에서 글로벌하게 사용되는 DTO, 에러, 인터페이스 등에 수정사항이 생겨서 바꾸는 일이 무척 어려울 것이라고 예상된다.
***마이크로서비스 구조***가 아니고 ***모노리틱한 구조***라면 **static** 으로 선언해서 관리하면 해당 사항이 없는 일이긴 하다...
