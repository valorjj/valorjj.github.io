...
class Solution {
    ...
    public long[] solution(int[][] program) {
        ...
        // 실행큐는
        // 우선순위 > 호출순서 기준으로 정렬
        PriorityQueue<int[]> execQueue = new PriorityQueue<>((o1, o2) -> {
        ...
        while(true) {
            ...
            // while 문 종료
            if (waitQueue.isEmpty() && execQueue.isEmpty() && singleProgramRunTime == 0) {
                break;
            }
            ...
            while(!waitQueue.isEmpty() && waitQueue.peek()[1] == totalProgramRunTime) {
                execQueue.add(waitQueue.poll());
            }

            // 실행되고 있는 프로그램이 없고,
            // 아직 실행할 프로그램이 남은 경우 프로그램을 실행한다.
            if (singleProgramRunTime == 0 && !execQueue.isEmpty()) {
                // 가장 우선순위가 높은 프로그램을 찾는다.
                // 우선순위 큐에 저장시켰기 때문에,
                // 우선순위에 따라
                int[] currentProgram = execQueue.poll();
