...
google 관리형 인증서는 다음과 같이 생성하면 된다. `(예제)`
...
이 때, 응답 헤더에 쿠키를 추가하기 위해서는 `ResponseCookie` 를 사용하면 된다고 한다. 크롬 브라우저 정책이 `Lax` 로 바뀜에 따라서 HTTPS 통신이 가능한 경우, `httpSecure` 옵션을 주고 인증 및 인가 정보를 쿠키에 담아서 보내면 된다.
