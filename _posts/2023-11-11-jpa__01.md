...
`Store` 엔티티와 연관관계에 묶인 `Employee` 객체도 함께 `SELECT` 를 하는 한방 쿼리가 나간다. 따라서 DB Connection Pool 에 여러 번 접근하는 과정을 생략하고 딱 한번에 전부 가져온다.
...
하지만, 실질적으로 성능 테스트를 해보고 우아한 기술팀이 내린 결론은 다음과 같다.
